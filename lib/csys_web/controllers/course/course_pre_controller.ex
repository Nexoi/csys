defmodule CSysWeb.CoursePreController do
  ...
  swagger_path :current_table do
    get "/api/pre_tables"
    description "获取全部已选课程（预选课表）"
    response 200, "success"
    response 204, "empty"
  end

  swagger_path :chose do
    post "/api/pre_courses/{course_id}"
    description "选课"
    parameters do
  ...
  swagger_path :cancel do
    PhoenixSwagger.Path.delete "/api/pre_courses/{course_id}"
    description "退课"
    parameters do
      course_id :path, :integer, "课程id", required: true
    end
  ...
  def index(conn, params) do
    ...
    courses = if word do
      ...
      |> conflict_append(current_user_id)
    else
  ...
  defp conflict_append(course_page, user_id) do
    ...
    %{
      ...
      entries: ConflictProcesser.enhance_conflict_courses(course_page.entries, nil, user_id)
    }
  end

  def chose(conn, %{"course_id" => course_id}) do
    current_term_id = CourseDao.find_current_term_id() # 预选当前学期课程
    current_user_id = get_session(conn, :current_user_id)
    ...
    if current_term_id do
      case ConflictProcesser.judge_dup(current_user_id, current_term_id, course_id) |> IO.inspect  do
        {:ok, msg} ->
          # IO.puts(msg)
          case CourseDao.chose_precourse(current_user_id, course_id) do
            {:ok, msg} ->
              conn
              |> json(%{message: msg})
            {:error, msg} ->
  ...
  def cancel(conn, %{"course_id" => course_id}) do
    ...
    if current_term_id do
      case CourseDao.cancel_precourse(current_user_id, course_id) do
        {:ok, msg} ->
          conn
          |> json(%{message: msg})
        {:error, msg} ->
  ...
  def current_table(conn, _) do
    ...
    if courses = CourseDao.list_course_tables(current_user_id) do
      conn
      |> render(TableView, "table.json", table: courses)
    else
      conn
      |> put_status(:bad_request)
      |> json(%{message: "Fetch Error"})
    end
  end
end
